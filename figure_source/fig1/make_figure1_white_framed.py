"""Create a white-background, journal-style VALID-OT framework schematic."""
...
def draw_panel_a(ax):
    ...
    rounded(ax, 0.010, 0.22, 0.135, 0.63, PAPER)
    ...
    # Finite intervention
    rounded(ax, 0.363, 0.22, 0.137, 0.63, PAPER, INTERVENTION)
    ...
    rounded(ax, 0.534, 0.17, 0.175, 0.73, PAPER, SOLVE)
    ...
    ax.text(0.8115, 0.775, "Row-plan responses", ha="center", va="top",
            transform=ax.transAxes, fontsize=7.0, fontweight="bold", color=INK)
    rounded(ax, 0.762, 0.535, 0.099, 0.135, PAPER, INTERNAL, lw=0.7, radius=0.010, z=2)
    ...
    rounded(ax, 0.762, 0.345, 0.099, 0.135, PAPER, INTERVENTION, lw=0.7, radius=0.010, z=2)
    ...
    rounded(ax, 0.741, 0.025, 0.174, 0.105, PAPER, EXTERNAL, lw=0.7, radius=0.010, z=2)
...
def draw_panel_b(ax):
    ...
    rounded(ax, 0.683, 0.12, 0.365, 0.74, PAPER, SOLVE, lw=0.85, radius=0.018)
    ...
    ax.text(0.8705, 0.250, "all applicable linear terms ×0.5",
            ha="center", va="center", transform=ax.transAxes, fontsize=5.2, color=MUTED)
    rounded(ax, 0.718, 0.145, 0.305, 0.060, PAPER, BOUNDARY, lw=0.65, radius=0.010, z=2)
    ax.text(0.8705, 0.175, "not a finite-difference path",
...
def draw_panel_c(ax):
    ...
    rounded(ax, 0.015, 0.47, 0.435, 0.42, PAPER, INTERNAL, lw=0.8, radius=0.018)
    ...
    ax.text(0.2325, 0.595, "Spearman · top-decile overlap\nNMAE",
    ...
    rounded(ax, 0.550, 0.47, 0.435, 0.42, PAPER, EXTERNAL, lw=0.8, radius=0.018)
    ...
    rounded(ax, 0.075, 0.115, 0.850, 0.205, PAPER, BOUNDARY, lw=0.8, radius=0.018)
    ax.text(0.500, 0.278, "Independent biological\nreference required", ha="center", va="top",
            transform=ax.transAxes, fontsize=6.6, fontweight="bold",
            color=BOUNDARY, linespacing=1.0)
    ax.text(
        0.500,
        0.175,
        "exact reoptimization is model-relative",
        ha="center",
        va="center",
        transform=ax.transAxes,
        fontsize=5.3,
...
def main():
    ...
    fig = plt.figure(figsize=(7.0, 5.40), facecolor=PAPER)
    gs = fig.add_gridspec(
        2,
        1,
    ...
    stem = OUT / "fig1_audit_design_white"
    ...
    try:
        ...
        fig.savefig(OUT / "fig1_audit_design_white_revised.pdf", bbox_inches="tight", facecolor=PAPER)
    plt.close(fig)
