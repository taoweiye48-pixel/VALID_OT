...
def draw_panel_a(ax):
    ...
    ax.text(0.6215, 0.775, "Complete reoptimization", ha="center", va="top",
            transform=ax.transAxes, fontsize=7.0, fontweight="bold", color=INK)
    ax.plot([0.565, 0.678], [0.710, 0.710], color=SOLVE, lw=1.1,
            transform=ax.transAxes, clip_on=False)
    solve_rows = [
    ...
    ax.text(0.8115, 0.775, "Row-plan responses", ha="center", va="top",
            transform=ax.transAxes, fontsize=7.0, fontweight="bold", color=INK)
    ax.plot([0.770, 0.8115], [0.710, 0.710], color=INTERNAL, lw=1.1,
            transform=ax.transAxes, clip_on=False)
    ax.plot([0.8115, 0.853], [0.710, 0.710], color=INTERVENTION, lw=1.1,
            transform=ax.transAxes, clip_on=False)
...
def draw_panel_b(ax):
    ...
    ax.plot([0.710, 1.020], [0.850, 0.850], color=SOLVE, lw=1.2,
            transform=ax.transAxes, clip_on=False)
    ...
    for xx, label in chips:
        rounded(ax, xx, 0.445, 0.086, 0.105, SOLVE_PALE, SOLVE, lw=0.65, radius=0.010, z=2)
...
def draw_panel_c(ax):
    ...
    ax.plot([0.055, 0.410], [0.890, 0.890], color=INTERNAL, lw=1.2,
            transform=ax.transAxes, clip_on=False)
    ...
    ax.plot([0.590, 0.945], [0.890, 0.890], color=EXTERNAL, lw=1.2,
            transform=ax.transAxes, clip_on=False)
    ax.plot([0.500, 0.500], [0.485, 0.870], color=HAIR, lw=0.55,
            transform=ax.transAxes, clip_on=False)
...
def main():
    ...
    # Use an explicit stem so an Illustrator-locked earlier white PDF cannot
    # leave a mixed set in which raster and vector files show different
    # revisions.
    stem = OUT / "fig1_audit_design_white_frameless"
    ...
    try:
        ...
        fig.savefig(OUT / "fig1_audit_design_white_frameless_revised.pdf", bbox_inches="tight", facecolor=PAPER)
    plt.close(fig)
